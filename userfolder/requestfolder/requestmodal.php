...
if (isset($_GET['q'])) {
    $searchTerm = $_GET['q'];
    error_log("Search Term Received: $searchTerm"); // Debugging: Log the search term    $sql = "SELECT asset_name, reg_no, category, description, quantity FROM asset_table WHERE asset_name LIKE ? LIMIT 10"; // Fetch matching assets including quantity
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit-request'])) {
    $assetName = isset($_POST['asset-name']) ? $_POST['asset-name'] : '';
    ...
    if (!empty($requestedBy)) {
        $userSql = "SELECT department, CONCAT(firstname, ' ', lastname) AS full_name FROM user_table WHERE username = ?";
        $userStmt = $conn->prepare($userSql);
        $userStmt->execute([$requestedBy]);
    ...
    if (empty($assetName) || empty($regNo) || empty($qty) || empty($category) || empty($date)) {
        ...
        $checkQuantitySql = "SELECT quantity FROM asset_table WHERE asset_name = ?";
        ...
        if ($row) {
            ...
            if ($availableQuantity <= 0) {
                ...
                try {
                    ...
                    // Insert the request
                    $insertSql = "INSERT INTO request_table (reg_no, asset_name, description, quantity, category, department, assigned_employee, requested_by, request_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    ...
                    if ($success) {
                        ...
                        if ($updateStmt->execute([$qty, $assetName])) {
                            $conn->commit();
                            echo "<script>alert('Request submitted successfully and asset quantity updated!');</script>";
                        } else {
                            $conn->rollBack();
                            echo "<script>alert('Failed to update asset quantity.');</script>";
                        }
                    } else {
                        $conn->rollBack();
