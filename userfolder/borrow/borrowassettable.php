...
// Get admin_borrow_for name from user_table if not set in session
if (!isset($_SESSION['admin_borrow_for']) || empty($_SESSION['admin_borrow_for'])) {
    $user_sql = "SELECT firstname, lastname FROM user_table WHERE username = :username LIMIT 1";
    $user_stmt = $conn->prepare($user_sql);
    $user_stmt->bindParam(':username', $_SESSION['username'], PDO::PARAM_STR);
    $user_stmt->execute();
    $user_row = $user_stmt->fetch(PDO::FETCH_ASSOC);
    if ($user_row && (!empty($user_row['firstname']) || !empty($user_row['lastname']))) {
        $_SESSION['admin_borrow_for'] = trim($user_row['firstname'] . ' ' . $user_row['lastname']);
    }
}
...
$sql = "SELECT * FROM borrow_table 
        WHERE department = '{$_SESSION['department']}' AND borrow_by = '{$_SESSION['username']}'
...
// Get employee full name from user_table
$user_fullname = '';
$user_sql = "SELECT firstname, lastname FROM user_table WHERE username = :username LIMIT 1";
$user_stmt = $conn->prepare($user_sql);
$user_stmt->bindParam(':username', $_SESSION['username'], PDO::PARAM_STR);
...
$user_row = $user_stmt->fetch(PDO::FETCH_ASSOC);
if ($user_row && (!empty($user_row['firstname']) || !empty($user_row['lastname']))) {
    $user_fullname = trim($user_row['firstname'] . ' ' . $user_row['lastname']);
}

// Calculate total number of returned assets
$total_sqls = "SELECT COUNT(*) as total FROM borrow_table WHERE department = :department AND (employee_name = :fullname OR admin_borrow_for = :fullname) AND returned = 1";
$total_stmts = $conn->prepare($total_sqls);
$total_stmts->bindParam(':department', $_SESSION['department'], PDO::PARAM_STR);
$total_stmts->bindParam(':fullname', $user_fullname, PDO::PARAM_STR);
$total_stmts->execute();
...
$returnedSql = "SELECT * FROM borrow_table 
                WHERE department = :department AND (employee_name = :fullname OR admin_borrow_for = :fullname) AND returned = 1 
                ORDER BY returned_date DESC 
                LIMIT :offset, :limit";
$returnedStmt = $conn->prepare($returnedSql);
$returnedStmt->bindParam(':department', $_SESSION['department'], PDO::PARAM_STR);
$returnedStmt->bindParam(':fullname', $user_fullname, PDO::PARAM_STR);
            ...
            <table class="table shadow-lg table-striped table-bordered table-hover">
                ...
                if ($stmt->rowCount() > 0) {
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        // Procurement Status - Read only display
                        echo "<td>";
                        if ($row['pro_status'] == 0) {
                            echo "<span class='badge badge-warning'>Pending</span>";
...
function updateStatus(type, id, status, button) {
    ...
    $.ajax({
        url: '/asset_management/hodfolder/borrow/update_status.php',
        type: 'POST',
        data: data,
        dataType: 'json',
...
function returnAsset(id, button) {
    if (confirm('Are you sure you want to return this asset?')) {
        $.ajax({
            url: '/asset_management/profolder/borrow/return_asset.php',
            type: 'POST',
            data: { return_id: id },
            dataType: 'json',
            success: function(response) {
