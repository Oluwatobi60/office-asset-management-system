...
if (isset($_GET['q'])) {
    ...
    try {
        $sql = "SELECT asset_name, reg_no, category, description, quantity FROM asset_table WHERE asset_name LIKE ? LIMIT 10";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit-request'])) {
    try {
        ...
        try {
            ...
            $checkStmt = $conn->prepare("SELECT quantity FROM asset_table WHERE asset_name = ?");
            ...
            $updateStmt = $conn->prepare("UPDATE asset_table SET quantity = quantity - ? WHERE asset_name = ?");
            $updateStmt->execute([$qty, $assetName]);
            ...
            error_log("Error in borrow request: " . $e->getMessage());
        }
    } catch (PDOException $e) {
