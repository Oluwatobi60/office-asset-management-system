 ...
 * and                         echo "<td>";
                        echo "<a href='borrow/deleteborrow.php?id=" . $row['id'] . "'><i class='fa fa-trash' style='color:red'></i></a> ";
                        echo "<a href='borrow/viewborrow.php?id=" . $row['id'] . "'><i class='fa fa-eye'></i></a>";
                        echo "</td>";us update functionality for HOD and Procurement approvals
 */ -->
...
$total_sql = "SELECT COUNT(*) as total FROM borrow_table";
...
$sql = "SELECT * FROM borrow_table 
        ORDER BY pro_status ASC, hod_status ASC, borrow_date DESC 
        LIMIT :offset, :limit";
...
$total_sqls = "SELECT COUNT(*) as total FROM borrow_table WHERE returned = 1";
            ...
            <table class="table shadow-lg table-striped table-bordered table-hover">
                ...
                if ($stmt->rowCount() > 0) {
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "</td>";                        // Procurement Status
                        echo "<td>";
                        if ($row['pro_status'] == 0) {
                            if ($row['hod_status'] == 0) {
                                echo "<button class='btn btn-warning' disabled title='HOD approval required'>Waiting for HOD...</button>";
                            } else if ($row['hod_status'] == 1) {
                                echo "<div class='btn-group'>";
                            ...
                            echo "<button class='btn btn-secondary' disabled>N/A</button>";
                        }
                        echo "</td>";

                        // Return button cell
                        echo "<td>";
                        if ($row['pro_status'] == 1 && $row['hod_status'] == 1) {
                            if (!isset($row['returned']) || $row['returned'] == 0) {
                                echo "<button onclick='returnAsset(" . $row['id'] . ", this)' class='btn btn-info'>Not Yet Return</button>";
                            } else {
                                echo "<span class='badge badge-success'>Returned</span>";
                            }
                        } else if ($row['hod_status'] == 2 || $row['pro_status'] == 2) {
...
function updateStatus(type, id, status, button) {
    // Check if procurement is trying to act before HOD approval
    if (type === 'pro') {
        const hodStatus = $(button).closest('tr').find('td:nth-child(6) button').hasClass('btn-success');
        if (!hodStatus) {
            showNotification('HOD needs to approve first before Procurement can take action', 'error');
            return;
        }
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                if (status === 1) {
                    ...
                    showNotification(response.message || 'Approved successfully', 'success');
                } else if (status === 2) {
                    $(button).closest('.btn-group').html(
                        "<button class='btn btn-danger' disabled>Rejected</button>"
                    );
                    showNotification(response.message || 'Rejected successfully', 'success');
                }
                
                // Get the row element
                let row = $(button).closest('tr');
                
                // Check if both HOD and PRO are approved
                let hodApproved = row.find('td:nth-child(6) button').hasClass('btn-success');
                let proApproved = row.find('td:nth-child(7) button').hasClass('btn-success');
                
                // If both are approved, show return button
                if (hodApproved && proApproved) {
                    let returnCell = row.find('td:nth-child(8)');
                    if (returnCell.is(':empty')) {
                        returnCell.html(`<button onclick='returnAsset(${id}, this)' class='btn btn-info'>Return</button>`);
                    }
                }            } else {
        ...
        error: function(xhr, status, error) {
            ...
            if (!navigator.onLine) {
                ...
                errorMessage = 'Update service not found. Please check the URL path.';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotification(response.message || 'Asset returned successfully', 'success');
                    ...
                    showNotification(response.message || 'Error returning asset: ' + (response.error || 'Unknown error'), 'error');
                }
            },
            error: function(xhr, status, error) {
                // Reset button state
                ...
                if (!navigator.onLine) {
                    ...
                    errorMessage = 'Return service not found. Please check the URL path.';                } else if (xhr.status === 500) {
                    errorMessage = 'Server error occurred. Please try again.';
                }
